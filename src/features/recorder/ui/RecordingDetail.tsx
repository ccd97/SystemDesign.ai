...
export function RecordingDetail({
  ...
  useEffect(() => {
    ...
    const filteredElements = finalScene.elements.filter((el) => {
      if (!el) return false;
      if (el.isDeleted === true) return false;
      if (el.type === "text" && (!el.text || String(el.text).trim() === "")) return false;
      return true;
    });
    ...
    const rawTheme = String(storedAppState?.theme || "light");
    const theme = rawTheme === "dark" ? "dark" : "light";
    let viewBackgroundColor = String(storedAppState?.viewBackgroundColor || "");
    if (!viewBackgroundColor || (theme === "dark" && (viewBackgroundColor === "#ffffff" || viewBackgroundColor === "#fff"))) {
      viewBackgroundColor = theme === "dark" ? "#121212" : "#ffffff";
    }

    const generateSnapshot = async () => {
      try {
        const restored = restoreElements(filteredElements as unknown as Parameters<typeof restoreElements>[0], null);
        const canvas = await exportToCanvas({
          elements: restored as unknown as Parameters<typeof exportToCanvas>[0]["elements"],
          appState: {
            viewBackgroundColor,
            theme,
